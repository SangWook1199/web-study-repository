...
/**
 * loadOnStartup =1 웹 어플리케이션이 실행이 시작되는 시점에 가장 우선적으로 위 옵션이 명시된 서블릿을 초기화합니다 서블릿
 * 라이프 사이클 구조상 클라이언트의 첫 요청 시점에 서블릿이 초기화 작업을 하므로 웹 어플리케이션에서 중요한 역할을 하는 서블릿(초기화
 * 작업이 많은 서블릿 포함) 은 위 옵션이 유용합니다
 */
