...
/**
 * Model2 MVC의 Controller 역할을 수행하는 서블릿 - 클라이언트의 요청을 분석 - 분석 내용에 따라 Model 과 연동 -
 * 연동 결과를 View 와 공유하기 위한 작업 - 제어 이동 방식을 결정하여 응답을 위한 View 로 이동 시킨다
 */
...
public class FindNewsByIdServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 정수형 변환: 웹 통신시에는 텍스트로 모든 정보가 인식되므로 별도의 형변환이 필요
		...
		// (forward 선택 이유는 request 와 response 가 유지되어 이동되므로)
