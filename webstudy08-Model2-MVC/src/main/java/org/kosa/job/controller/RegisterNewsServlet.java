...
public class RegisterNewsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 공부를 위해 일단 forward로 제어 해본다
		// forward로 처리한 후 client가 새로고침하면 뉴스가 다시 등록되는 문제가 발생
//		request.getRequestDispatcher("/news-register-result.jsp").forward(request, response);
		// 이후 redirect 로 이동후 비교해본다
		// forward 는 웹 컨테이너 상에서 이동되는 반면 redirect는 응답시에 브라우저에게 
		// 지정한 url로 재요청하여 응답 결과를 보라는 방식
