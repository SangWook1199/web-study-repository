 ...
 *  :시스템 상에서 단 한 번 객체를 생성하여
 *  공유해서 사용하는 디자인 패턴 -> 스프링에서 기본적으로 객체를 운용(매니징)하는 방식
 *  why? 서버 자원의 효율성: 불필요하게 반복적으로 객체를 여러개 생성하지 않음으로 자원 절약
 *       실제로 스프링 bean(자바 컴포넌트), 서블릿이 컨테이너에 의해 싱글톤으로 만들어진다
 *  
 *  private Constructor
 *  private static 인스턴수 변수
 *  public static 메서드
 */
public class NewsService {
	private static NewsService instance = new NewsService();
	// 외부에서 객체 생성 못하게 원천 봉쇄
	private NewsService() {
