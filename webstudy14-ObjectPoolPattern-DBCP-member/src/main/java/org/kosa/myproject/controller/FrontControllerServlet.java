...
/**
 * 웹 어플리케이션은 다양한 서버 제품군에서 일관된 서비스를 진행가능
 */
@WebServlet("/front")
public class FrontControllerServlet extends HttpServlet {
	...
	/**
	 * work flow 클라이언트 요청(command)을 받아온다 요청 처리 담당 객체(Controller)를 받아온다
	 * HandlerMapping Factory를 통해서 요청을 처리할 담당 Controller를 실행한다 실행 후 반환된 정보를 이용해
	 * forward 또는 redirect 방식으로 응답하게 View 로 제어를 이동한다 예외 공동정책: 예외 발생시 사용자에게는 점검중 페이지를
	 * 제공하고 시스템 콘솔에서는 예외 발생 경로와 메세지를 출력한다
	 * 
	 */
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (path.trim().startsWith("redirect")) {
				...
				request.getRequestDispatcher(path.trim()).forward(request, response);
			}
