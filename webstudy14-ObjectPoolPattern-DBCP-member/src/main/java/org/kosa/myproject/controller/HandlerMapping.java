...
/**
 * Factory Design Pattern: 객체 생성을 전담하는 설계 패턴 구체적 객체 생성부를 캡슐화 HandlerMapping:
 * 요청(request or command) 처리를 위한 각 컨트롤러 객체 생성을 전담하는 Factory class Spring 에서 동일한
 * 이름으로 동일한 역할을 함 일반적으로 Factory 는 시스템상에서 단 하나만 존재하여 운영되므로 Singleton Pattern을
 * 적용한다
 */
